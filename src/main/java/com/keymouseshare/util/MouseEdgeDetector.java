...
public class MouseEdgeDetector {
    ...
    private static final double BASE_THRESHOLD = 5.0; // 基础阈值（像素）
    private static final double VELOCITY_FACTOR = 0.1; // 速度因子
    private static final long ANTI_FALSE_TRIGGER_TIME = 100; // 防误触时间（毫秒）
    private static final double IGNORE_BUFFER_ZONE = 165.0; // 忽略中间区域（像素）
     ...
     * 检查鼠标是否在屏幕边缘
     * 
     * @param x 鼠标X坐标（虚拟桌面坐标）
     * @param y 鼠标Y坐标（虚拟桌面坐标）
     * @return 如果鼠标在屏幕边缘且满足触发条件，返回将被唤醒鼠标的ScreenInfo对象，否则返回null
     */
    public static ScreenInfo isAtScreenEdge(double x, double y) {
        ...
        for (Map.Entry<String, ScreenInfo> entry : screens.entrySet()) {
            ...
            if (isAdjacentScreen(currentScreen, screen)) {
                ...
                if (direction != EdgeDirection.NONE) {
                    logger.debug("鼠标在当前屏幕的边缘方向: " + direction);
        ...
        if (targetScreen != null) {
            ...
            if (shouldTrigger) {
                logger.info("触发边缘检测，将唤醒设备: " + targetScreen.getDeviceIp() + 
                ...
                lastTime = currentTime;
                lastVelocity = velocity;
                if(targetDirection.name().equals("LEFT")){
                    targetScreen.setX(x-10.0 - targetScreen.getVx());
                }
                if(targetDirection.name().equals("RIGHT")){
                    targetScreen.setX(x+10.0 - targetScreen.getVx());
                }
                if(targetDirection.name().equals("TOP")){
                    targetScreen.setX(y-10.0 - targetScreen.getVy());
                }
                if(targetDirection.name().equals("BOTTOM")){
                    targetScreen.setX(y+10.0 - targetScreen.getVy());
                }
     ...
     * @return 鼠标移动速度（像素/毫秒）
     */
    private static double calculateVelocity(double x, double y, long currentTime) {
        if (lastTime == 0) {
    ...
    private static boolean isAdjacentScreen(ScreenInfo screen1, ScreenInfo screen2) {
        ...
        logger.debug("屏幕1边界: left=" + left1 + ", right=" + right1 + ", top=" + top1 + ", bottom=" + bottom1);
        logger.debug("屏幕2边界: left=" + left2 + ", right=" + right2 + ", top=" + top2 + ", bottom=" + bottom2);
        ...
        logger.debug("水平相邻: " + horizontalAdjacent + ", 垂直相邻: " + verticalAdjacent);
    ...
    private static EdgeDirection getEdgeDirection(double x, double y, ScreenInfo screen, double threshold) {
        ...
        logger.debug("获取边缘方向: x=" + x + ", y=" + y + ", 屏幕边界: left=" + left + 
